...
import pandas as ps
import numpy as np
import os, collections, re, random, getpass
import vk_api
import csv, json, sys
import random
...
def help():
    page = """
        Hello. This is sir_liba. Best friend of data sciencer, which loves vk.com :)
        So, what this liba can give to you? Vk parsing and processing EDA with parsed data.
        liba have two classes:
            ...
            EdaMaker
                ...
                VkParse creates compatible csv. If you want process EDA with other csv, you have to change your csv for
                make it compatible.
        ...
        P.P.S. It is assumed that you are familiar with the basics of data analysis and the python language.
        This library is NOT a high-level data analysis tool.
        It is designed to speed up the writing of the code for the project 'sirius/call-center',
...
class EdaMaker:
    ...
    def help(self):
        page = """
            Hello. This is EDAMAKER part of sir_liba. It will make some EDA on your vkontakte dataframe.
            It will be wonderfull,
            if your dataframe was created by sir_liba too. So, your dataframe MUST have these columns:
                'group_name' -- name of group, from which row of dataframe was loaded
                'text' -- text of publication
                When you make object of this class, you will see construction of your dataframe.
            ...
            So, this class have only three useable for you methods:
                ...
                save(arg_name)
                    will save last modification of your dataframe, storaged in memory to disk

                makewords(ret=False)
                    will make BagOfWords(BOW) for dataframe, stored in memory.
                    will make WordByFrequency(WBF) for dataframe, stored in memory.
                    WILL NOT clean dataframe.
                    if 'ret' was set as True, will return BOW and WBF, so prepare two variables..

                __init__(filename, text_series='text') (when you create object)
                    will initiale EDA maker with filename. text_series set by default as 'text'.
                    if in your dataframe text data to make EDA is in column with different name,
                    you have to set text_series as that name.

            How to use it?
            ok, you have dataframe. Lets make class object:
                eda = EdaMaker(your_csv_to_eda)
                    you will see configuration of dataframe. you MUST CHECK IT TI HAVE 'group_name' and 'text'
            ...
            Tips:
                if you working under jupyter :), you can check your dataframes:
                    ...
                    eda.stopwords -- list of stopwords, that will be removed during cleantext()
                     If you want more, see code))))
            """
        print(page)

    def __init__(self, filename, arg_dropnan=True, text_series='text', processed_series='processed_text', length_series='length', group_series='group_name', cat_series='category_name'):
        csv = ps.read_csv(filename, encoding='utf-8')

        try:
            test_data = csv.dropna(subset=[text_series])[text_series] ## if text_series column exists
            test_string = test_data[len(test_data)//2]
            if test_string.isnumeric():
                print("WARN: your test data seems to NOT be plain text")
        except KeyError:
            print("ERR: column '",text_series,"' with data to EDA does not exists.")
    ...
    def lemm(self):
        ...
        #        serie.append(' '.join([stemmer.stem(i) for i in text.split(' ')]))
    ...
    def cleantext(self, arg_regexp=r'', arg_stopwords=['russian'], lemm=True, normalize=True, remshort=True, delstops=True):
        ...
        self.origin_series = self.current.dropna(subset=[self.text_series])[self.text_series]
